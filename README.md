...
Em vez disso, `students[].active_chat_time_sec` é calculado por [`scripts/parse_logibot.py`](scripts/parse_logibot.py) a partir dos timestamps reais das mensagens: para cada sessão de chat, é o intervalo entre a primeira e a última mensagem. Esse cálculo só é possível para os 60 alunos que têm sessão de chat registrada — os outros 14 alunos ativos só fizeram quiz, sem chat.

**Sessões reaproveitadas em dias diferentes são excluídas do cálculo.** O app reaproveita a mesma `chat_session_id` quando o aluno volta a uma conversa antiga em vez de abrir uma nova — em 4 sessões isso fez o aluno mandar a primeira mensagem num dia e voltar só 1-3 dias depois (spans de 46h a 73h), o que não é tempo de uso, é o gap entre dois dias de teste. Há um corte nítido nos dados: a sessão "normal" mais longa é de 53 min, e a próxima já pula para 46h — por isso `parse_logibot.py` descarta do cálculo qualquer sessão com mais de 2h de intervalo entre a primeira e a última mensagem (só desse cálculo de tempo; o resto dos dados do aluno - quiz, outras mensagens - é mantido normalmente).
...
📖 **[Guia dos gráficos](charts/RESUMO.md)** — explicação simples de cada gráfico e do que cada métrica significa, com as imagens embutidas. Também disponível em PDF: [`charts/RESUMO.pdf`](charts/RESUMO.pdf).
...
.venv/bin/python scripts/charts_resumo_to_pdf.py  # regera charts/RESUMO.pdf
```

`charts_resumo_to_pdf.py` converte o `charts/RESUMO.md` para PDF usando o Google Chrome instalado (modo `--headless`, sem precisar de pandoc/wkhtmltopdf/weasyprint) — se o Chrome não estiver em `/Applications`, ajuste `CHROME_CANDIDATES` no início do script.
...
├── charts/
│   ├── RESUMO.md                     # guia dos gráficos (texto + imagens)
│   ├── RESUMO.pdf                    # o mesmo guia, em PDF
...
│   └── charts_resumo_to_pdf.py       # charts/RESUMO.md -> charts/RESUMO.pdf
